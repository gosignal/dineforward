...
export function useQuery<D, V>(
  ...
  const [result, setResult] = useState(() => {
    ...
    const data = initialResult.loading || initialResult.partial ? null : (initialResult.data as D);
...
export function useSubscription<D>(
  ...
  useEffect(() => {
    const subscription = observable.current.subscribe(event => setResult(event.data));
    return () => subscription.unsubscribe();
  });
  return result;
}
...
export function useQueryWithSubscription<QD, QV, SD>(
  ...
  configuredSubscription: (client: ApolloClient<any>) => Observable<{ data: SD }>,
